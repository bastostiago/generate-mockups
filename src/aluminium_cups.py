...
def run_aluminium_cups(app, kind_of_c, del_files='Y'):
    ...
    if files_to_process:
        for file_to_process in files_to_process:
            ...
            # 2 Cups and Art
            if kind_of_c in (1, 2):
                psd_file = os.path.join(DIR_PSD_CUPS, 'aluminium_cups_art.psd')
                ...
                if opened:
                    ...
                    img_name = f"{file_name}_1_aluminium_cup.jpg"
            ...
            if kind_of_c == 1:
                ...
                # Cup Side 1
                psd_file = os.path.join(DIR_PSD_CUPS, 'aluminium_cup_1.psd')
                opened = app.openPSD(psd_file)
                if opened:
                    app.update_layer_image('cup1_image', file_to_process)
                    img_name = f"{file_name}_2_aluminium_cup.jpg"
                    app.exportJPEG(img_name, dir_to_save)
                    if CONFIG['DEFAULT']['GenerateNoBackgroundImages'] == 'yes':
                        img_name = f"{file_name}_2_aluminium_cup.png"
                ...
                # Cup Side 2
                psd_file = os.path.join(DIR_PSD_CUPS, 'aluminium_cup_2.psd')
                opened = app.openPSD(psd_file)
                if opened:
                    app.update_layer_image('cup2_image', file_to_process)
                    img_name = f"{file_name}_3_aluminium_cup.jpg"
                    app.exportJPEG(img_name, dir_to_save)
                    if CONFIG['DEFAULT']['GenerateNoBackgroundImages'] == 'yes':
                        img_name = f"{file_name}_3_aluminium_cup.png"
